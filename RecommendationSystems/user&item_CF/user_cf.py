...
class UserCF:
    ...
    def split_data(self, data, test_ratio=0.2):
        ...
        for user_id, items in data.items():
            # 获取用户的所有评分
            item_ratings = list(items.items())
            # 随机打乱
            random.shuffle(item_ratings)
            # 计算测试集大小
            test_size = max(1, int(len(item_ratings) * test_ratio))
            
            # 分割数据
            test_items = item_ratings[:test_size]
            train_items = item_ratings[test_size:]
    ...
    def fit(self, train_file, validation_ratio=0.2):
        """训练模型，并返回验证集"""
        # 读取训练数据
        ...
        # 分割数据为训练集和验证集
        train_data, validation_data = self.split_data(self.user_ratings, validation_ratio)
        
        # 使用训练集数据
        self.user_ratings = train_data
        self.item_ratings = defaultdict(dict)
    ...
    def evaluate(self, validation_data):
        ...
        for user_id, items in validation_data.items():
