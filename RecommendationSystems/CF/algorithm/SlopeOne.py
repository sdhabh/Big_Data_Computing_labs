...
class SlopeOne:
            ...
            for item2, users2 in item_users.items():
                if item1 < item2:  # 只计算上三角矩阵，避免重复计算
                    ...
                    if common_users:  # 只处理有共同用户的物品对
                        # 计算评分差异
                        diffs = [
                            self.item_ratings[item1][u] - self.item_ratings[item2][u]
            ...
            for item2, users2 in item_users.items():
                if item1 < item2:
                    ...
                    if common_users:
                        ...
                        freq = len(common_users)
                        self.diff_matrix[item1][item2] = mean_diff
                        self.diff_matrix[item2][item1] = -mean_diff
                        self.freq_matrix[item1][item2] = freq
                        self.freq_matrix[item2][item1] = freq
    ...
    def predict(self, user_id, item_id):
        ...
        for rated_item, rating in user_ratings.items():
            if (
                rated_item in self.diff_matrix
                and item_id in self.diff_matrix[rated_item]
            ):
